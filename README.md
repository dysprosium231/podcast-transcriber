...
- **说话人区分**（可选）：基于 [sherpa-onnx](https://github.com/k2-fsa/sherpa-onnx)，纯 CPU，跟两个转录引擎都能配合。转录完之后单独对同一段音频再分析一遍，只给每句话加一个说话人标签，不改动已经生成的时间戳，字幕同步不受影响。**「说话人数量」留空靠自动判断这条路准确率上限不高**——实测一集双人播客，自动判断把同一个人拆成了十几到几十个不同的人；同一集音频改成明确填「2」，直接精确聚成2个人。知道确切人数的话（很多播客本来就是固定几个主播）务必手动填，别指望自动判断
...
4. **说话人区分**（可选）：想用的话勾选「启用说话人区分」，点「下载此模型」，知道这期音频有几个人说话的话务必把「说话人数量」填上——留空自动判断的准确率不高
